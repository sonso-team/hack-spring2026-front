...
export class Background
{
    ...
    private getEdgeInset (width: number): number
    {
        if (width <= 768)  return 0;
        if (width <= 1024) return Math.max(width * 0.08, 8);
        if (width <= 1440) return Math.max(width * 0.14, 10);
    ...
    private drawStars (horizonY: number, totalHeight: number)
    {
        ...
        for (const star of this.stars)
        {
            ...
            const xRaw     = star.nx * this.width + progress * star.drift * this.width;
            const x        = ((xRaw % this.width) + this.width) % this.width;

            // Верхняя половина
            this.graphics.fillStyle(0xddeeff, alpha);
            this.graphics.fillCircle(x, progress * horizonY, star.size);

            // Нижняя половина - отзеркалирована
            this.graphics.fillStyle(0xddeeff, alpha);
            this.graphics.fillCircle(x, totalHeight - progress * (totalHeight - horizonY), star.size);
        }
    }

    private drawHorizonGlow (horizonY: number)
    {
        const layers: Array<{ halfH: number; alpha: number }> = [
            { halfH: 2,   alpha: 0.30 },
            { halfH: 8,   alpha: 0.14 },
            { halfH: 22,  alpha: 0.07 },
            { halfH: 55,  alpha: 0.04 },
        ...
        for (const { halfH, alpha } of layers)
        {
            this.graphics.fillStyle(0xaa66ff, alpha);
            this.graphics.fillRect(0, horizonY - halfH, this.width, halfH * 2);
        }
    }
    ...
    )
    {
        for (let i = 0; i < rowCount; i++)
        {
            ...
            this.graphics.lineBetween(leftX, horizonY + perspective * floorHeight,  rightX, horizonY + perspective * floorHeight);
            this.graphics.lineBetween(leftX, horizonY - perspective * ceilingHeight, rightX, horizonY - perspective * ceilingHeight);
        }
    }
}
